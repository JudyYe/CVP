...
def save_pose(strip=1, maxnum=16, draw=False, save=False):
    ...
    for v in range(1, kAll + 1):
        ...
        bone_vid, x1, y1, x2, y2 = crop_window(bone_vid, valid, w, h, m=35)
        crop_vid(v, nframes, x1, y1, x2, y2, save)
        ...
        if save:
            save_file = os.path.join(bone_dir, '%04d_cropJoint.npz' % v)
            if not os.path.exists(os.path.dirname(save_file)):
                os.makedirs(os.path.dirname(save_file))
...
def crop_window(bone_vid, valid, w, h, m=0):
    """
    :param bone_vid: (dt, Joint, 2)
    :param valid: (Joint)
    :return:
    """
    X = bone_vid[:, :, 0]
...
def crop_vid(v, nfrs, x1, y1, x2, y2, save):
    for f in range(nfrs):
...
def draw_skeleton(img, x, y, valid, m=30):
    x = x.astype(np.int)
    y = y.astype(np.int)
