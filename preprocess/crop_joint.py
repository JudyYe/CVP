...
def vis(vid_name, X, Y, nframes, act, strip=4, idx=None):
    ...
    for f in idx:
        ...
        draw_skeleton(img, X[f], Y[f])  # 13 * 2
...
def save_pose(strip, maxnum=16):
    ...
    for v in range(1, kAll + 1):
        ...
        bone_vid, x1, y1, x2, y2 = crop_window(bone_vid, w, h, m=25)
        crop_vid(v, nframes, x1, y1, x2, y2)

        vis('%04d' % v, bone_vid[:, :, 0], bone_vid[:, :, 1], nframes, act, strip=1)
...
def crop_window(bone_vid, w, h, m=0):
    ...
    x2 = min(np.max(bone_vid[:, :, 0]) + m, w)
    y2 = min(np.max(bone_vid[:, :, 1]) + m, h)
    r = max(x2 - x1, y2 - y1) / 2
...
def draw_skeleton(img, x, y, m=30):
    x = x.astype(np.int)
    y = y.astype(np.int)
